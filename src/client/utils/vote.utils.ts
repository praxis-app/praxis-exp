...
export const filterVotesByType = (votes: VoteBadgesFragment['votes']) => {
  const agreements = votes.filter((vote) => vote.voteType === VoteTypes.Agreement);
  const reservations = votes.filter((vote) => vote.voteType === VoteTypes.Reservations);
  const standAsides = votes.filter((vote) => vote.voteType === VoteTypes.StandAside);
