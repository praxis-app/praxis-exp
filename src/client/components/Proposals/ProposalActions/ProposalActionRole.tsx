...
const ProposalActionRole = ({ actionType, preview, ratified, role, ...boxProps }: Props) => {
  ...
  const [
    getSelectedUsers,
    { data: selectedUsersData, loading: selectedUsersLoading, error: selectedUsersError },
  ...
  useEffect(() => {
    ...
    const userIds = role.members.map((member) => (member as ProposalActionRoleMemberInput).userId);
    getSelectedUsers({
      variables: { userIds },
    });
  ...
  const roleToChange = 'groupRole' in role ? role.groupRole : selectedRoleData?.groupRole;

  const oldName = ratified && 'oldName' in role ? role.oldName : roleToChange?.name;
  const oldColor = ratified && 'oldColor' in role ? role.oldColor : roleToChange?.color;
  ...
  return (
    <Box marginBottom={preview ? 0 : 2.5} {...boxProps}>
      <Accordion expanded={showRole} onChange={() => setShowRole(!showRole)} sx={accordionStyles}>
        <AccordionSummary>
        ...
        <AccordionDetails sx={{ marginBottom: isDesktop ? 2 : 3 }}>
          {!isRoleChange && !members?.length && !hasPermissions && (
            <Typography>{t('proposals.prompts.emptyPermsAndMembers')}</Typography>
          )}

          <Grid columns={isDesktop ? 12 : 4} columnSpacing={3} rowSpacing={1} container>
            {isRoleChange && (
              <>
                {isChangingName && (
                  <Grid item xs={6}>
                    <Typography fontFamily="Inter Bold" fontSize={15} paddingTop={0.2} gutterBottom>
                      {t('proposals.labels.name')}
                    </Typography>

                    <Flex marginBottom={isDesktop ? 0 : 1} sx={changeStyles}>
                      <ChangeIcon changeType={ChangeType.Remove} sx={{ marginRight: '1ch' }} />
                    ...
                    <Flex sx={changeStyles}>
                      <ChangeIcon changeType={ChangeType.Add} sx={{ marginRight: '1ch' }} />
                      <Typography color="primary" fontSize="inherit" marginRight="0.25ch">
                        {name}
                      </Typography>
                    </Flex>
                ...
                {isChangingColor && (
                  <Grid item xs={6}>
                    ...
                    <Flex sx={changeStyles}>
                      <ChangeIcon changeType={ChangeType.Remove} sx={{ marginRight: '0.8ch' }} />
