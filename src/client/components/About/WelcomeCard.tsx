...
const WelcomeCard = () => {
  ...
  const signUpPath = data?.isFirstUser ? NavigationPaths.SignUp : `/signup/${inviteToken}`;

  return (
    <Card>
      <CardHeader title={t('prompts.welcomeToPraxis')} sx={{ paddingBottom: 0.75 }} />
      ...
      {inviteToken && (
        <CardActions>
          <Link href={signUpPath}>
            <Button sx={{ color: 'text.primary' }}>{t('users.actions.signUp')}</Button>
          </Link>
        </CardActions>
      )}
    </Card>
